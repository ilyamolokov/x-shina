...
export const CatalogCategory = (props: CatalogCategoryProps) => {
  const { categoryProperties, categoryChildren, onForwardClick, onBackClick } =
    props

  return (
    <div className={styles.catalogCategoryConteiner}>
      {!!categoryProperties.id && (
        <BaseButton onClick={onBackClick}>{'< Назад'}</BaseButton>
      )}
      <h1>{categoryProperties.name}</h1>
      {categoryChildren.map(({ id, name, slug }) => {
        return (
          <BaseButton key={id} onClick={() => onForwardClick(slug)}>
            {name} {' >'}
          </BaseButton>
        )
      })}
